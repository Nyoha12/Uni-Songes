...
source précédemment validée `77f3ebcd8385606a381013ce289bd63f084ee7db` est sans
conflit. Aucun défaut fonctionnel nouveau ne nécessite de modifier la View, le
Twig ou le helper : seule cette documentation est actualisée dans cette reprise.

Terminal 1 / PR #113 possède seul DDEV et le checkout servant ; Terminal 3
prépare uniquement les fichiers de #94. Cette exclusivité est une règle de
coordination, pas une dépendance technique de #90 à #113. Aucun autre worktree
n'est consulté ni modifié, notamment #82. Aucun DDEV, Docker, Drush, navigateur,
Mailpit, VPS, PayPal ou Google n'est utilisé ici. Le runtime reste non exécuté et
la PR #90 reste en brouillon ; sa reprise nécessitera une nouvelle autorisation.
...
livraison ou une boutique physique existe.

## Intégration de la base finale

Le rebase conserve le commit fonctionnel de cette PR comme unique commit au-dessus
de la base vérifiée. Depuis la base précédente `5b8e80c`, `release/prod` a reçu
#98, #95, #87, #103 puis #104. Ces fusions ne modifient aucun des trois fichiers
fonctionnels de #90 ni `composer.lock`. Les contrats fusionnés qui touchent le
shell public restent conservés :
...
| PR #100 | La configuration synchronisée du bloc de messages reste dans `content/-8` et la suggestion inline conserve la destination `[data-drupal-messages]` dans `main` ; l'état actif reste à vérifier en runtime. |
...
Les changements de contraste #95 et de mouvement #98 présents sur la nouvelle
base sont hérités ; #90 n'y ajoute aucun CSS ou JavaScript. Leur rendu combiné
avec le panier n'est pas revendiqué comme testé dans cette reprise statique.
...
Le contrat de la [PR #114](https://github.com/Nyoha12/Uni-Songes/pull/114), encore
ouverte et en brouillon au head `b9406a6ed7157ee964d35b12a41db1d6e2968743`, a été
relu uniquement en lecture seule. Son
[audit du parcours actuel](https://github.com/Nyoha12/Uni-Songes/blob/b9406a6ed7157ee964d35b12a41db1d6e2968743/docs/functional/online-payment-slot-handoff-2026.md#L212)
confirme que le créneau ne suit pas le panier classique. Le
[futur ordre dédié](https://github.com/Nyoha12/Uni-Songes/blob/b9406a6ed7157ee964d35b12a41db1d6e2968743/docs/functional/online-payment-slot-handoff-2026.md#L1093)
est prévu hors du CartProvider général. Les décisions de copie DEC-19 et de
traitement des achats classiques DEC-32 restent ouvertes. #90 ne reprend aucun
claim, finalizer, blocage d'achat ou migration de vieux paniers de ce contrat.
La copie actuelle guide le visiteur sans présenter ces mécanismes futurs comme
disponibles ; elle reste inchangée.
...
être reconstruit lors d'une future validation autorisée. Les commandes ci-dessus
décrivent cette procédure ; elles ne sont pas exécutées ici, même en dry-run.
Le helper exige le runtime Drupal/Drush verrouillé, sans configuration, service
ou code provenant de #113. Ses snapshots, transactions, verrous et rollback
existants sont conservés sans reconstruction.

## Validation statique

La reprise réutilise les validations acquises sur les sources inchangées et
limite ses nouveaux contrôles aux quatre fichiers de #90 et à leurs contrats
d'intégration. Aucun audit global ni nouveau framework de tests n'est ajouté.

Aucun runtime ni installation de dépendances n'est utilisé dans cette reprise.

### Preuves acquises le 2 septembre 2026

Les résultats suivants correspondent à `77f3ebc`, sur la base `5b8e80c` ; les
comptages globaux sont historiques, pas de nouveaux tests de la base actuelle :
...
### Contrôles ciblés du 7 septembre 2026

L'identité octet pour octet des trois fichiers fonctionnels et de `composer.lock`
avec `77f3ebc` conserve la validité du parsing YAML, de la compilation Twig et de
l'audit du rendu Commerce 3.3.2. Les anciens outils temporaires Symfony/Twig ne
sont plus disponibles dans cet environnement ; ils ne sont ni réinstallés ni
empruntés à un autre worktree. Aucune nouvelle compilation n'est revendiquée.

Les nouveaux contrôles vérifient :

- la base distante `9ef3d4a`, le rebase sans conflit et les quatre fichiers exacts ;
- l'égalité de la View actuelle avec celle de la base après la seule substitution
  `Item` → `Article`, y compris UUID, dépendances, champs et actions natifs ;
- la source YAML revue, SHA-256
  `f2e4fa2e87f4fc3509578fd978298ed9d7f357907b27eb84ea168521e2eb1b82`, et le Twig
  inchangé, SHA-256 `ec3c36b6cca914e3055e0fe624b84fe1836a16e3916b3613416cf11873243284` ;
- le H2, la navigation nommée, les trois destinations exactes et l'absence de
  message artificiel ou de promesse de réservation/livraison/boutique ;
- les contrats #84/#99/#100 et le graphe de bibliothèques inchangés, l'absence de
  modification d'en-tête et de dépendance technique à #113 ;
- la compatibilité documentaire avec #114, relue indépendamment sans reprendre
  ses mécanismes futurs ;
- `bash -n`, ShellCheck 0.9.0 et le lint du PHP embarqué avec PHP 8.2.33 ;
- le diff, UTF-8/NFC, les marqueurs de conflit et le scan de secrets à forte
  confiance sur les quatre fichiers ; aucun import ou SQL brut ajouté.

Commandes de lint exécutées depuis la racine du worktree :

```bash
bash -n drupal/scripts/apply-cart-ux-2026.sh
shellcheck drupal/scripts/apply-cart-ux-2026.sh
awk 'BEGIN { print "<?php" } /^IFS= read -r -d .*PHP_CODE/ { capture=1; next } capture && /^PHP$/ { exit } capture { print }' \
  drupal/scripts/apply-cart-ux-2026.sh | php -l /dev/stdin
git diff --check origin/release/prod --
git diff --exit-code 77f3ebcd8385606a381013ce289bd63f084ee7db -- \
...
Les relectures indépendantes du contrat #114 et de l'intégration à la base
actuelle ne trouvent aucun défaut nécessitant de modifier les sources du panier.

## Matrice runtime différée

Cette matrice reste à exécuter avant de sortir #90 du brouillon. Terminal 1 /
#113 possède seul DDEV et le checkout servant ; cela n'impose aucune dépendance
technique ou ordre de fusion entre les PR. La libération de DDEV ne déclenchera
aucun test automatiquement : une nouvelle autorisation runtime est nécessaire.
Aucun résultat runtime de #90 n'est déclaré réussi dans cette reprise.
